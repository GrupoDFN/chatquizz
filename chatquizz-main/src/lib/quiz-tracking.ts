...
// Função interna para enviar ao Redis (a ponte que aguentou os 1000 VUs)
const sendToQueue = (table: string, data: any) => {
  fetch('https://weizgspqnjhqxycnkvvh.supabase.co/functions/v1/push-to-redis', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ table, ...data }),
    keepalive: true,
  }).catch(e => console.warn(`Redis Queue Error (${table}):`, e));
};

export async function trackQuizView(quizId: string, sessionId: string) {
  // Agora envia para o Redis em vez do banco direto
  sendToQueue("quiz_views", { quiz_id: quizId, session_id: sessionId });
}
